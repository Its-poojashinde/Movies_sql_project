...
select movie_name,lead_star from movies_sql where lead_star='salman khan';
...
select movie_name,budget from movies_sql order by budget desc limit 1;
...
in the dataset.*/
select movie_name,(revenue-budget) as profit from movies_sql order by profit asc limit 5;

/*25. Retrieve the names of top 5 hit movies in 
the dataset.*/
select movie_name,(revenue-budget) as profit from movies_sql order by profit desc limit 5;
...
given the biggest hit movie*/
 select director, movie_name,(revenue-budget) as profit from movies_sql
 where lead_star='shahrukh khan'  order by profit desc limit 1;